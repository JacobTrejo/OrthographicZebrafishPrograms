...
def generateRandomVideoConfigurationFastStates(fishInView, fishInEdges, OverlappingFish, overlapMarker, numFrames, video_id):
    """
    Returns:
    ...
    # generating fishes that are completely in the view, based on their keypoints
    for fish_id in range(fishInView):
        # Loop to keep on searching in case we randomly generate a fish that was not in the view
        # Generating random fishVect
        np.random.seed(fish_id * (video_id + 1) * 1000)
