...
def return_overlapping_fishVectList(fishVectList, fishesToOverlapIdices):
    """
    Function that returns the fishVectList of the fishes that are going to be overlapped. It also makes sure that non-overlapping fish trajectories don't go outside the image frame
    :param fishVectList: the list of fishVects
    :param fishesToOverlapIdices: the indices of the fishVectList that are going to be overlapped
    :return: the fishVectList of the fishes that are going to be overlapped
    """
    numFrames = len(fishVectList[0])
    paddingForFishAtEdge = 10 # Padding for the fish at the edge of the frame that rendered fish is not clipped
    # While loop to make sure that the fish don't go outside the frame
    while True:
        # Randomly sample frame numbers at which the overlap happens between the given fish
        overlappingFrames = []
        for i in range(len(fishesToOverlapIdices)):
            overlappingFrames.append(np.random.randint(0, numFrames))
        
        for i in range(len(fishesToOverlapIdices)):
            # Make the overlapping frame the origin for every trajectory of overlapping fish
            x_at_overalp, y_at_overlap = fishVectList[fishesToOverlapIdices[i]][overlappingFrames[i], 2:4]
            fishVectList[fishesToOverlapIdices[i]][:, 2] -= x_at_overalp
            fishVectList[fishesToOverlapIdices[i]][:, 3] -= y_at_overlap
        
        # Shift all overlapping trajectories so that the fish don't go outside the frame
        x_max = np.NINF
        y_max = np.NINF
        x_min = np.PINF
        y_min = np.PINF
        for i in range(len(fishesToOverlapIdices)):
            x_min_, y_min_ = np.min(fishVectList[fishesToOverlapIdices[i]][:, 2:4], axis=0)
            x_max_, y_max_ = np.max(fishVectList[fishesToOverlapIdices[i]][:, 2:4], axis=0)
            if x_min_ < x_min:
                x_min = x_min_
            if y_min_ < y_min:
                y_min = y_min_
        ...
        x_shift_min = np.random.uniform(paddingForFishAtEdge - x_min, imageSizeX - x_max - paddingForFishAtEdge)
        y_shift_min = np.random.uniform(paddingForFishAtEdge - y_min, imageSizeY - y_max - paddingForFishAtEdge)
        x_shift_max = np.random.uniform(x_max - (imageSizeX - paddingForFishAtEdge), x_min - paddingForFishAtEdge)
        y_shift_max = np.random.uniform(y_max - (imageSizeY - paddingForFishAtEdge), y_min - paddingForFishAtEdge)

        for i in range(len(fishesToOverlapIdices)):
            if x_min < paddingForFishAtEdge:
                fishVectList[fishesToOverlapIdices[i]][:, 2] += x_shift_min
            if y_min < paddingForFishAtEdge:
                fishVectList[fishesToOverlapIdices[i]][:, 3] += y_shift_min
            
            if x_max > imageSizeX - paddingForFishAtEdge:
                fishVectList[fishesToOverlapIdices[i]][:, 2] -= x_shift_max
            if y_max > imageSizeY - paddingForFishAtEdge:
                fishVectList[fishesToOverlapIdices[i]][:, 3] -= y_shift_max
            
            
            #fishVectList[fishesToOverlapIdices[i]][:, 2] -= x_min
            #fishVectList[fishesToOverlapIdices[i]][:, 3] -= y_min

        if x_max - x_min < imageSizeX - 2 * paddingForFishAtEdge or y_max - y_min < imageSizeY - 2 * paddingForFishAtEdge:
            fishesNotOverlappingIndices = [i for i in range(len(fishVectList)) if i not in fishesToOverlapIdices]
            for i in range(len(fishesNotOverlappingIndices)):
                x_min_, y_min_ = np.min(fishVectList[fishesNotOverlappingIndices[i]][:, 2:4], axis=0)
                x_max_, y_max_ = np.max(fishVectList[fishesNotOverlappingIndices[i]][:, 2:4], axis=0)
                fishVectList[fishesNotOverlappingIndices[i]][:, 2] -= x_min_
                fishVectList[fishesNotOverlappingIndices[i]][:, 3] -= y_min_
                traj_width = x_max_ - x_min_
                traj_height = y_max_ - y_min_
                x_shift = np.random.randint(0, imageSizeX - traj_width - 2 * paddingForFishAtEdge)
                y_shift = np.random.randint(0, imageSizeY - traj_height - 2 * paddingForFishAtEdge)
                fishVectList[fishesNotOverlappingIndices[i]][:, 2] += x_shift + paddingForFishAtEdge
                fishVectList[fishesNotOverlappingIndices[i]][:, 3] += y_shift + paddingForFishAtEdge    
            break
...
def generateRandomConfiguration(fishInView, fishInEdges, OverlappingFish):
    ...
    for _ in range(fishInView):
        ...
        while True:
            ...
            xVect = np.zeros((22))
            fishlen = (np.random.rand(1) - 0.5) * 40 + averageSizeOfFish
    ...
    for overLappingFishIdx in range(OverlappingFish):
        ...
        while True:
            ...
            # Generating the fish
            xVect = np.zeros((22))
            fishlen = (np.random.rand(1) - 0.5) * 20 + averageSizeOfFish
            idxlen = np.floor((fishlen - 62) / 1.05) + 1
            seglen = 5.6 + idxlen * 0.1
            seglen = seglen[0]
            # seglen = 7.1
...
def generateRandomConfigurationFast(fishInView, fishInEdges, OverlappingFish):
    ...
    shouldBreak = False   # Break if you got a good overlappingFishVectList
    while True:
        shouldRunAgain = False  # Run again if overlappingFishVectList is bad

        overlappingFishVectListInstance = []
        ...
        for overLappingFishIdx in range(OverlappingFish):
            ...
            while True:
                ...
                if is_atleast_one_point_in_bounds(pts):
                    overlappingFishVectListInstance.append(fishVect)
                    break

        # Let's check if our instance is valid
        for fishVect in overlappingFishVectListInstance:
            if not doesThisFishInterfereWithTheAquarium(fishVect, overLappingFishVectList):
                overLappingFishVectList.append(fishVect)
            else:
                overlappingFishVectListInstance = []
...
def get_fish_params(params_path, num_fish):
    """
    params_path: Path of the 22-dimensional pose tracjetories for the 3-D dataset
    num_fish: Number of fish 
    """
    fish_params = loadmat(params_path)['x_all'][0]
    fishId = np.random.randint(0, fish_params.shape[0], num_fish)
    fish_params = fish_params[fishId]
    return fish_params


def generateRandomVideoConfigurationFastStates(fishInView, fishInEdges, OverlappingFish, overlapMarker, numFrames):
    """
    Returns:
    fish_params: a list of all fish containing pose dynamics over time
    FishVectList: a list of (24,) numpy arrays, each representing a fish size and pose in the view
                The first two elements of the array represent the size and the z-plane (used for overlapping the rendered fish)
                The next 22 elements represent the x, y, z, theta (heading), phi (inclination), and gamma (roll) of the fish
    """
    averageSizeOfFish = Config.averageSizeOfFish
    fishVectList = []
    ...
    for _ in range(fishInView):
        ...
        z = 1
        # seglen = 7.1
        #fish_params = np.random.rand(4, 120, 22) # 4 fish, 120 frames, 22 keypoints for testing
        numFishes = Config.maxFishesInView
        fish_params = get_fish_params(Config.fishParamsPath, numFishes) # numFishes = maxFishesInView because we are generating a constant fish number. Consider making it random in the future
        x0, y0 = np.random.randint(0, imageSizeX), np.random.randint(0, imageSizeY)
        while True:
            fish_id = np.random.randint(0, len(fish_params))
            if fish_params[fish_id].shape[0] > numFrames:
                break
        first_frame = np.random.randint(0, fish_params[fish_id].shape[0] - numFrames)
        pose = fish_params[fish_id][first_frame:first_frame + numFrames, ...]
        xVect = np.zeros((numFrames, 22))
        dtheta = -pose[:, 4:12]
        dphi = pose[:, 13:21]            
        xVect[:, :2] = pose[:, :2] * Config.lenToPixelScaling
        xVect[:, 2] = 0. # A dummy z-coordinate
        xVect[:, 3] = pose[:, 3]
        xVect[:, 4:12] = dtheta            
        xVect[:, 12] = pose[:, 12]
        xVect[:, 13:21] = dphi
        xVect[:, 21] = pose[:, 21]
        fishVect = np.zeros((numFrames, 24))
        fishVect[:, 0] = seglen
        fishVect[:, 1] = z
        fishVect[:, 2:] = xVect
        fishVectList.append(fishVect)
   ...
   # for state in OverlappingFish:
    ...
    fishVectList = return_overlapping_fishVectList(fishVectList, fishesToOverlapIdices)
    return fishVectList
...
def generateRandomConfigurationFastStates(fishInView, fishInEdges, OverlappingFish, overlapMarker):
    ...
    for _ in range(fishInView):
        ...
        while True:
            ...
            dtheta = angles_array[angles_array_idx, 0:8]
            dphi = angles_array[angles_array_idx, 8:16]
            xVect[:3] = [x, y, np.random.randint(0, imageSizeY)] # A dummy z-coordinate
            xVect[3] = np.random.rand(1)[0] * 2 * np.pi
            xVect[4:12] = dtheta            
            xVect[12] = np.random.normal(loc=0, scale=phi0_scale)
            xVect[13:21] = dphi
    ...
    # return fishVectList, overLappingFishVectList
    temp = fishVectList + overLappingFishVectList
    del_indices = random.sample(range(len(temp)), len(temp) - fishInView)
    fishVectList = [item for i, item in enumerate(temp) if i not in del_indices]
