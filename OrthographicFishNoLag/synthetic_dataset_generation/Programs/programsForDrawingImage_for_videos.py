...
def draw_anterior_b(seglen, theta, phi, gamma, dh1, dh2, dimension, size_lut, random_params):
    XX = size_lut
    YY = size_lut
    ZZ = size_lut

    # Distance between eyes
    d_eye = Config.d_eye * seglen * random_params['d_eye_mult']
    # # Note: temporary
    # d_eye = seglen * 1.18
    # Relative position of eyes, belly and head on the ball and stick model
    c_eyes = Config.c_eyes * random_params['c_eyes_mult']
    c_belly = Config.c_belly * random_params['c_belly_mult']
    c_head = Config.c_head * random_params['c_head_mult']
    ...
    eyes_br = Config.eyes_br * random_params['eyes_br_mult']
    belly_br = eyes_br * Config.belly_br * random_params['belly_br_mult']
    head_br = belly_br * Config.head_br * random_params['head_br_mult']
    # # # NOTE: temporary
    # eyes_br = 255 * .9
    # # belly_br = 255 * .
    # # head_br = 255 * .28
    ...
    rand1_eye = random_params['rand1_eye']
    rand2_eye = random_params['rand2_eye']
    rand3_eye = random_params['rand3_eye']
    rand1_belly = random_params['rand1_belly']
    rand2_belly = random_params['rand2_belly']
    rand1_head = random_params['rand1_head']
    rand2_head = random_params['rand2_head']
...
def gen_lut_b_tail(n, seglen, d1, d2, t, random_params):
    size_lut = 45
    ...
    random_number_size = random_params['random_number_size']
...
def f_x_to_model_centered(x, seglen, random_params):
    randomize = 1
    ...
    dh1 = pt[0, 1] - np.floor(pt[0, 1])
    dh2 = pt[1, 1] - np.floor(pt[1, 1])
    fish_anterior, eye1_c, eye2_c, _, _, _, _ = draw_anterior_b(seglen, theta_0, phi_0, gamma_0, dh1, dh2, 2, size_lut, random_params)
    min_Y = pt[1, 1] - (size_half) + np.argwhere(np.sum(fish_anterior, axis=1) > 0)[0]
    max_Y = pt[1, 1] - (size_half) + np.argwhere(np.sum(fish_anterior, axis=1) > 0)[-1] + 1
    min_X = pt[0, 1] - (size_half) + np.argwhere(np.sum(fish_anterior, axis=0) > 0)[0]
    max_X = pt[0, 1] - (size_half) + np.argwhere(np.sum(fish_anterior, axis=0) > 0)[-1] + 1
    ...
    for ni in range(0, 7):
        ...
        dt2 = pt[1, n] - coor_t2
        tailpix = imblank
        tail_model = gen_lut_b_tail(ni, seglen, dt1, dt2, theta[n], random_params)
        tailpix[
        int(np.maximum(0, coor_t2 - (size_half - 1))): int(np.minimum(imageSizeY, 1 + coor_t2 + (size_half - 1))),
        int(np.maximum(0, coor_t1 - (size_half - 1))): int(
            np.minimum(imageSizeX, 1 + coor_t1 + (size_half - 1)))] = tail_model[
...
def f_x_to_model_bigger(x, seglen, random_params, imageSizeX=648, imageSizeY=488):
    """
    ...
    """
    graymodel, pt, new_origin = f_x_to_model_centered(x, seglen, random_params)
