...
def draw_anterior_b(seglen, theta, phi, gamma, dh1, dh2, dimension, size_lut, randomize):
    XX = size_lut
    YY = size_lut
    ZZ = size_lut

    # Distance between eyes
    d_eye = Config.d_eye * seglen * (randomize * normrnd(1, 0.05) + (1 - randomize))
    # # Note: temporary
    # d_eye = seglen * 1.18
    # Relative position of eyes, belly and head on the ball and stick model
    c_eyes = Config.c_eyes * (randomize * normrnd(1, 0.05) + (1 - randomize))
    c_belly = Config.c_belly * (randomize * normrnd(1, 0.05) + (1 - randomize))
    c_head = Config.c_head * (randomize * normrnd(1, 0.05) + (1 - randomize))
    ...
    projection = np.maximum(np.maximum(np.maximum(project_eye1, project_eye2), project_belly), project_head)
    projection = np.clip(projection, 0, 255)
...
def gen_lut_b_tail(n, seglen, d1, d2, t, randomize):
    ...
    random_number_size = randomize * normrnd(1., 0.1) + (1 - randomize) * 0.5
    ballsize = random_number_size * np.array([3, 2, 2, 2, 2, 1.5, 1.2, 1.2, 1]) * Config.ball_size
    # Thickness of the sticks in the model
    thickness = random_number_size * np.array([7, 6, 5.5, 5, 4.5, 4, 3.5, 3]) * Config.tail_thickness
    # Brightness of the tail
    b_tail = np.array([0.7, 0.55, 0.45, 0.40, 0.32, 0.28, 0.20, 0.15]) / 1.5 * Config.tail_br
...
def f_x_to_model_centered(x, seglen, randomize, cam='bottom'):
    ...
    pt = np.array(pt)
    pt = pt + np.tile(hp, (1, 10))
    pt_2D = projection_function_3D_to_2D(pt, cam)

    # Construct headpix (larval anterior)
    size_lut = 75
    size_half = (size_lut + 1) / 2

    dh1 = pt_2D[0, 1] - np.floor(pt_2D[0, 1])
    dh2 = pt_2D[1, 1] - np.floor(pt_2D[1, 1])
    fish_anterior, eye1_c, eye2_c, _, _, _, _ = draw_anterior_b(seglen, theta_0, phi_0, gamma_0, dh1, dh2, dimension, size_lut, randomize)
    min_Y = pt_2D[1, 1] - (size_half) + np.argwhere(np.sum(fish_anterior, axis=1) > 0)[0]
    max_Y = pt_2D[1, 1] - (size_half) + np.argwhere(np.sum(fish_anterior, axis=1) > 0)[-1] + 1
    min_X = pt_2D[0, 1] - (size_half) + np.argwhere(np.sum(fish_anterior, axis=0) > 0)[0]
    max_X = pt_2D[0, 1] - (size_half) + np.argwhere(np.sum(fish_anterior, axis=0) > 0)[-1] + 1

    new_origin = np.array([[np.minimum(np.min(pt_2D[0, :]), min_X)], [np.minimum(np.min(pt_2D[1, :]), min_Y)]])
    ...
    new_origin[0] = custom_round(pt_2D[0, 0]) - custom_round(smallImageSizeX / 2)
    new_origin[1] = custom_round(pt_2D[1, 0]) - custom_round(smallImageSizeY / 2)

    imageSizeX = np.maximum(np.uint16(np.max(pt_2D[0, :])), max_X) - new_origin[0]
    imageSizeY = np.maximum(np.uint16(np.max(pt_2D[1, :])), max_Y) - new_origin[1]
    ...
    #eye2_c[0] = eye2_c[0] - (size_half - 1) + pt_2D[0, 1]
    #eye2_c[1] = eye2_c[1] - (size_half - 1) + pt_2D[1, 1]
    pt_2D = np.concatenate([pt_2D, eye1_c_2D, eye2_c_2D], axis=1)
    imblank = np.zeros((int(imageSizeY), int(imageSizeX)), dtype=np.uint8)
    headpix = imblank.copy()
    bodypix = imblank.copy()
    coor_h1 = np.floor(pt_2D[0, 1])
    coor_h2 = np.floor(pt_2D[1, 1])
    headpix[int(np.maximum(0, coor_h2 - (size_half - 1))): int(np.minimum((imageSizeY), 1 + coor_h2 + (size_half - 1))),
    int(np.maximum(0, coor_h1 - (size_half - 1))): int(
        np.minimum((imageSizeX), 1 + coor_h1 + (size_half - 1)))] = fish_anterior[
                                                                    int(np.maximum((size_half - 1) - coor_h2, 0)): int(
    ...
    # Construct tailpix (larval posterior)
    theta = get_theta_from_2D_keypoints(pt_2D[:, :10])
    size_lut = 45
    size_half = (size_lut + 1) / 2
    for ni in range(0, 7):
        n = ni + 2
        coor_t1 = np.floor(pt_2D[0, n])
        dt1 = pt_2D[0, n] - coor_t1
        coor_t2 = np.floor(pt_2D[1, n])
        dt2 = pt_2D[1, n] - coor_t2
        tailpix = imblank
        tail_model = gen_lut_b_tail(ni, seglen, dt1, dt2, theta[n], randomize)
        tailpix[
        int(np.maximum(0, coor_t2 - (size_half - 1))): int(np.minimum(imageSizeY, 1 + coor_t2 + (size_half - 1))),
    ...
    # Combine headpix and tailpix into a single image
    graymodel = np.maximum(headpix, normrnd(1, 0.1) * bodypix)
    return graymodel, pt_2D, new_origin


def f_x_to_model_bigger(x, seglen, randomize, cam='bottom', imageSizeX=648, imageSizeY=488):
    """
    ...
    """
    graymodel, pt_2D, new_origin = f_x_to_model_centered(x, seglen, randomize, cam)
