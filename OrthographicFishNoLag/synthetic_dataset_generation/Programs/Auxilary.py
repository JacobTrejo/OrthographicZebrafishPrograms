...
def mergeGreys(grays, depths):
    ...
    alpha = np.array([1 / grays.shape[0] for _ in range(grays.shape[0])])
    alpha = alpha[:, np.newaxis, np.newaxis]
    mergedGrays = np.float32(mergedGrays)
    mergedGrays = np.sum(alpha * mergedGrays, axis=0)
    mergedGrays = np.uint8(mergedGrays)
    #mergedGrays = np.max(grays, axis=0)
...
def add_noise_static_noise(im, filter_size=None, sigma=None, noise_mean=None, noise_var=None):
    ...
    if noise_var is None:
        noise_var = (np.random.rand() * 60 + 20) / 255 ** 2
    im = imGaussNoise(im, noise_mean, noise_var)                      
    # Converting Back
    if maxGray != 0:
...
def add_noisy_spots(im, fish_list, pvar=None):
    imageSizeY, imageSizeX = im.shape[:2]
    averageAmountOfNoisySpots = Config.averageAmountOfNoisySpots
    if pvar is None:
        pvar = np.random.poisson(averageAmountOfNoisySpots)
    if pvar > 0:
        for i in range(1, int(np.floor(pvar + 1))):
            idxListOfSpottedFishes = [idx for idx, fish in enumerate(fish_list) if
                                        fish.is_valid_fish]
            amountOfPossibleCenters = len(idxListOfSpottedFishes)
            ...
            for centerIdx in range(amountOfCenters):
                center = np.zeros((2))
                shouldItGoOnAFish = True if np.random.rand() > 0.90 else False 
                if shouldItGoOnAFish:
                    fish = (fish_list)[idxListOfSpottedFishes[centerIdx]]
                ...
                randi1 = (2 * np.random.randint(1, high=5)) 
                randi2 = (2 * np.random.randint(1, high=5)) 
                se = cv.getStructuringElement(cv.MORPH_ELLIPSE, (randi1, randi2))
                zeros_mat = cv.dilate(zeros_mat, se)
                finalVar_mat += zeros_mat

            im = im / 255
            scaling_for_spots = 0.5 * (np.random.rand())
            im += (scaling_for_spots * finalVar_mat)
            im = im * 255
    return im


def add_patchy_noise(im, fish_list, pvar=None):
    imageSizeY, imageSizeX = im.shape[:2]
