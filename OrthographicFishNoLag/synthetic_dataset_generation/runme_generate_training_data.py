...
from Programs.Aquarium_variable_fish_num import Aquarium
from Programs.programsForGeneratingFishFrames import x_seglen_to_3d_points, addBoxes
import os
import shutil
...
def genData(idx):
    aquarium = Aquarium(idx, cam='side')
    aquarium.draw()
    aquarium.save_image()
...
if __debug__:
    # # Passing in amounts
    print('k')
    #aquarium = Aquarium(0, cam='side', fishInAllViews=3, fishInEdges=1, overlapping=2)
    aquarium = Aquarium(0, cam='side')
    # aquarium.draw()
    # aquarium.save_image()
    # aquarium.save_annotations()
