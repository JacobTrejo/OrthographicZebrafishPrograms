...
def draw_anterior_b(seglen, theta, phi, gamma, dh1, dh2, dimension, size_lut, randomize):
    ...
    belly_br = Config.eyes_br * Config.belly_br * (randomize * normrnd(1, 0.1) + (1 - randomize))
    head_br = Config.belly_br * Config.head_br * (randomize * normrnd(1, 0.1) + (1 - randomize))
    # # # NOTE: temporary
    # eyes_br = 255 * .9
    # # belly_br = 255 * .
    # # head_br = 255 * .28
    ...
    #projection = customUint8(np.maximum(np.maximum(np.maximum(project_eye1, project_eye2), project_belly), project_head))
