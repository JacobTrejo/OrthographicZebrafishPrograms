...
def getResnetCoors(cutouts, boxList):
    ...
    for i, im in enumerate(loader):
        ...
        pose_recon = list(pose_recon)
        poseReconList += pose_recon

    correctCoors = []
    # Sending back the coordinates to where they came from
    for poseIdx in range(len(poseReconList)):
        pose = np.copy(poseReconList[poseIdx])
...
while True:
    ...
    if len(frames) >= myBatchSize or fc >= amountOfFrames:
        startingIdx = fc - len(frames)
        masterBoxList = []
        masterCutoutList = []
        ...
        for result in results:
            ...
            #print('amount of boxes: ', len(boxList)) 
            mask = [isBoxFarFromEdge(box) for box in boxList]
            boxList = boxList[mask] 
            masterBoxList.append(boxList)

        # Lets get the cutouts
        for offsetIdx, boxList in enumerate(masterBoxList):
            bgsubIm =  bgsubIms[startingIdx + offsetIdx] 
            cutouts = []
            for box in boxList:
            ...
            masterCutoutList.append(cutouts)

        # Lets put them in a format to feed Resnet
        cutouts = []
        for cutoutList in masterCutoutList: cutouts += cutoutList
        boxes = []
        for boxList in masterBoxList: 
            boxes += list(boxList)

        coorList = getResnetCoors(cutouts, boxes)
        # Lets arrange the coorList into the frames they came from
        count = 0
        masterCoorList = []
        for boxList in masterBoxList:
            amount = len(boxList)
            coors = coorList[count:count + amount]
            count += amount
            masterCoorList.append(coors)
        
        for offsetIdx, coorList in enumerate(masterCoorList):
            bgsubIm =  bgsubIms[startingIdx + offsetIdx]
