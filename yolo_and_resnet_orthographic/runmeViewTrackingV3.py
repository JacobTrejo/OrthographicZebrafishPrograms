...
def deleteDuplicates(dataList):
    '''
        Deletes halucinations, the ones were NMS doest take care of
    '''
                ...
                for offsetIdx, otherFrameData in enumerate(newTrackingList4[frameIdx + 1:]):
                    ...
                    if len(needLinkList) > 0 and len(overlapFish2Add) > 0:
                        ...
                        if len(overlapFish2AddDummy) > 0:
                            ...
                            needLinkList = [[fishId, fish] for (fishId, fish) in needLinkList if
...
def checkIfRecorded(frameIdx, overlapP, comsMasterList):
    ...
    for (startIdx, comsList, amountList) in comsMasterList:
...
def connectOverlapBouts6(newTrackingList4, d2):
    print('Starting')
        ...
        for proxData in proximityList:
            ...
            if len(proxData['fishes']) > 1:
                ...
                for (fishInfoIds, startFrameIdx, endIdx) in startingData:
                    if len(fishInfoIds) == len(overlapFishInfoIds) and (
                            (frameIdx >= startFrameIdx) and (frameIdx <= endIdx)):
...
def connectOverlapBoutsLast(newTrackingList4, d2, comsMasterList2):
    proximityThreshold = 40
    #     proximityThreshold = 50
    linkList = []
    unlinkablelist = []
    maxHalucinationLen = 3
    for (startIdx, coms, amounts) in comsMasterList2:
        needLinkList = []
        overlapFishInfo = []
        com = coms[0]
        ...
        for (fishId, box, fish) in frameData:
            dist = np.sum((fish[2, :] - com) ** 2) ** .5
            if dist < proximityThreshold: overlapFishInfo.append([fishId, fish])

        if len(overlapFishInfo) < 2: continue
        overlapFishInfoIds = [fishId for (fishId, fish) in overlapFishInfo]

        for offsetIdx, overlapP in enumerate(coms[1:]):
...
def yoloFishEstimationPlusErasing(dataList2):
