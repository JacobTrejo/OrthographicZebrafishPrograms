...
from utils.group_multiple import HeatmapParserBatch
import utils.img
...
def post_process(det, mat_, trainval, c=None, s=None, resolution=None):
    ...
    cropped_preds = parser.parse(torch.stack((det, det), 0))
...
#         tmp[ii] = np.concatenate((tmp1[ii], tmp2[ii]),axis=0)
    ...
    #det = tmp['det'][0,-1]
    det = tmp1['det'][0,-1] 
    print('det type', det.shape)
    det = torch.minimum(det, 1)
    #global outDet
    #outDet = det
...
def runner(imgs):
    return func(0, config, 'inference', imgs=imgs)['preds']
...
for (imPath, coorPath) in zip(im_to_add, coor_to_add):
    ...
    image2 = image.clone()
    image2 = torch.stack((image2[0], image2[0], image2[0]), 2)
    
    image = image[0].numpy()
    image = np.stack((image, image, image), axis = 2)
    
    pred = do(image2, None, None)
