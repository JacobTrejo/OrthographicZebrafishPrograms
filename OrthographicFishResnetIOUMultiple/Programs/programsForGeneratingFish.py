...
from Programs.Auxilary import getBoundingBoxFromPoints, get_iou, createCircles, getIOUFromPoints
...
def getIOUFishVect(ogFishVect, depth = 3):
    averageSizeOfFish = Config.averageSizeOfFish
    maxOverlappingOffset = Config.maxOverlappingOffset

    fishVectToOverlap = ogFishVect
    ogSeglen = fishVectToOverlap[0]
    ogXVect = fishVectToOverlap[2:]
    ogPts = x_seglen_to_3d_points(ogXVect, ogSeglen)
    fishVect = None
    ...
    while True:
        ...
        isIOU = False
        newPts = x_seglen_to_3d_points(xVect, seglen)
        iou = getIOUFromPoints(ogPts, newPts)
        # if iou < .5:
        if iou > .5:
        ...
        if is_atleast_one_point_in_bounds(pts) and isIOU:
            break

    return fishVect

def getMergedBoundingBoxFromVect(fishVect1, fishVect2):
    seglen1 = fishVect1[0]
    xVect1 = fishVect1[2:]
    ...
    pts1 = x_seglen_to_3d_points(xVect1, seglen1)
    pts2 = x_seglen_to_3d_points(xVect2, seglen2)

    points, radii = createCircles(pts1)
    points2, radii2 = createCircles(pts2)
    ...
    sx, sy, bx, by = min(sx1, sx2), min(sy1, sy2), max(bx1, bx2), max(by1, by2)

    return [[sx, sy], [bx, by]]

def getIOUFromBoxYVect(box, fishVect):
    seglen = fishVect[0]
    xVect = fishVect[2:]

    pts1 = x_seglen_to_3d_points(xVect, seglen)
    points, radii = createCircles(pts1)

    bb = getBoundingBoxFromPoints(points, radii)
    return get_iou(box, bb)

# def generateRandomConfigurationIOU(overlappingFish, overlappingFish2):
#     overlappingFish = 1
#     overlappingFish2 = 0
#     averageSizeOfFish = Config.averageSizeOfFish
#     fishVect = None
...
#
#         break
#     fishVectList = [fishVect]
#
#     if overlappingFish > 0:
#         fishVect2 = getIOUFishVect(fishVect)
#         fishVectList += [fishVect2]
#         if overlappingFish2 > 0:
#             fishVect3 = getIOUFishVect(fishVect, depth=5)
#             fishVectList += [fishVect3]
#
#
#
#     # fishVectList = [fishVect, fishVect2, fishVect3]
#
#     return fishVectList

def getNonIOUFish(bb,ogFishVect, depth = 3):
    averageSizeOfFish = Config.averageSizeOfFish
    maxOverlappingOffset = Config.maxOverlappingOffset
    ...
    while True:
        ...
        isIOU = False
        newPts = x_seglen_to_3d_points(xVect, seglen)
        points, radii = createCircles(newPts)

        newBB = getBoundingBoxFromPoints(points, radii)
        iou = get_iou(bb, newBB)
...
def generateRandomConfigurationIOU(overlappingFish, overlappingFish2):
    ...
    if overlappingFish > 0:
        fishVect2 = getIOUFishVect(fishVect)
        fishVectList += [fishVect2]
        mergeBB = getMergedBoundingBoxFromVect(fishVectList[0], fishVectList[1])

        if overlappingFish2 > 0:
            # We generated the third fish
            # fishVect3 = getIOUFishVect(fishVect, depth=5)
            fishVect3 = getNonIOUFish(mergeBB, fishVect, depth=5)
            fishVectList += [fishVect3]
    ...
    for idx in range(len(fishVectList)):
        fishVect = fishVectList[idx]
        fishVect[1] = depths[idx]
        fishVectList[idx] = fishVect
