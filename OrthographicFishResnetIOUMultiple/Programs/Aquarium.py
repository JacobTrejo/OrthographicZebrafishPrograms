...
import numpy as np
import cv2 as cv
from Programs.programsForGeneratingFish import generateRandomConfiguration, generateRandomConfigurationNoLagChunks, generateRandomConfigurationFast, generateRandomConfigurationIOU
...
class Aquarium:
    ...
    def overloaded_constructor(self, **kwargs):
        ...
        if not wasAnAquariumPassed:
            if wasAnAquariumVariableDetected:
                ...
                #print('in view: ', fishesInView, 'in edge: ', fishesInEdge, 'overlapping: ', overlappingFish)
                #fishVectList = generateRandomConfigurationFast(fishesInView, fishesInEdge, overlappingFish)

                # This is the random Configuration for resnet fish
                fishInView = 1
                overlappingFish = 0
                secondOverlappingFish = 0
                if np.random.rand() > .5: overlappingFish = 1
                overlappingFish = 1
                if overlappingFish >= 1 and np.random.rand() > .5: secondOverlappingFish = 1

                fishVectList = generateRandomConfigurationIOU(overlappingFish, secondOverlappingFish)
        return fishVectList
    ...
    def save_image_resnet(self):
        imageSizeX, imageSizeY = Config.imageSizeX, Config.imageSizeY
        bb = self.fish_list[0].boundingBox
        bb2 = self.fish_list[1].boundingBox

        sx1, sy1, bx1, by1 = bb.smallX, bb.smallY, bb.bigX, bb.bigY
        sx2, sy2, bx2, by2 = bb2.smallX, bb2.smallY, bb2.bigX, bb2.bigY

        sx, sy, bx, by = min(sx1,sx2), min(sy1, sy2), max(bx1, bx2), max(by1, by2)

        maxBxOffset = (imageSizeX - 1) - bx
        maxByOffset = (imageSizeY - 1) - by
        maxSxOffset = sx
        maxSyOffset = sy

        bxOffset = np.random.randint(0, min(maxBxOffset, 10) + 1)
        byOffset = np.random.randint(0, min(maxByOffset, 10) + 1)
        sxOffset = np.random.randint(0, min(maxSxOffset, 10) + 1)
        syOffset = np.random.randint(0, min(maxSyOffset, 10) + 1)

        sx, sy, bx, by = sx - sxOffset, sy - syOffset, bx + bxOffset, by + byOffset

        gray = self.finalViews[0][0]
        cutout = gray[sy:by, sx:bx]
        strIdxInFormat = format(self.frame_idx, '06d')
        #cv.imwrite('test.png', cutout)
        cv.imwrite(Config.dataDirectory + 'images/im_' + strIdxInFormat + '.png', cutout)
        pts  = np.copy( self.fish_list[0].pts )
        pts2 = np.copy( self.fish_list[1].pts )
        pts[0,:] -= sx
        pts[1,:] -= sy
        pts2[0,:] -= sx
        pts2[1,:] -= sy
        pts = np.concatenate([pts, pts2], axis = 1)
        pts = torch.tensor(pts, dtype=torch.float32)
        torch.save(pts, Config.dataDirectory + 'coor_2d/ann_' + strIdxInFormat + '.pt')
